...
for (state, abbreviation) in zip(full_state_name, state_abbrev):
    print "{0}, {1}".format(state, abbreviation)
...
print "<select_name=\"state\">"
for (state, abbreviation) in zip(full_state_name, state_abbrev):
    print "\n \t <option_value='{0}'>{1}</options>".format(state, abbreviation)
print "</select>"
